...
Run the `run_af3_output_ter.sh` with the following positional arguments: working directory, path to the param file, ligand name (quoted as a string)
After preparing the filters.json file like this:
{
    "score": {
        "threshold": 1,
        "condition": false # "greater" or "less"
    } }

Binder has to be "condition" than "threshold" to be accepted.

```bash
sbatch 1.3_run_af3_output_ter.sh "/work/lpdi/users/eline/CID/b2_FUN_1Z9Y" "/work/lpdi/users/eline/CID/b2_FUN_1Z9Y/input/FUN.params" "FUN"
```
TODO : init pyr only once!!!
done merging af3 ternary csvs in  1994 seconds
# 

```bash
sbatch /work/lpdi/users/eline/CID_scoring/2_ter_pyr_scoring.sh "/work/lpdi/users/eline/CID/b2_FUN_1Z9Y" "/work/lpdi/users/eline/CID/b2_FUN_1Z9Y/input/FUN.params" "FUN"
```

Pyrosetta scoring and relaxation for 65 complexes: Script execution took: 5 hours, 29 minutes, 53 seconds
done pyr scoring in  27770 seconds
~8h for ~115 binders

plip:


## 5 - AF3 binary scoring: input prep and run then score

- run the ```run_af3_input_bin.sh``` with the following positional arguments: $WDIR, target_id, path to target_template
``` bash
sbatch /work/lpdi/users/eline/CID_scoring/run_af3_input_bin.sh /work/lpdi/users/eline/CID/b2_FUN_1Z9Y 1Z9Y /work/lpdi/users/eline/CID/b2_FUN_1Z9Y/input/1Z9Y_target_template.cif
```

- run af3 for the binary complexes:
``` bash
SDIR="/work/lpdi/users/eline/CID_scoring"
WDIR="/work/lpdi/users/eline/CID/b2_FUN_1Z9Y"
cd "$WDIR"
...
```bash
sbatch /work/lpdi/users/eline/CID_scoring/run_af3_output_bin.sh /work/lpdi/users/eline/CID/b2_FUN_1Z9Y
```


# 6 - Collecting all metrics and comparing ternary and binary complexes

In other words, does it help to have a ligand? 
