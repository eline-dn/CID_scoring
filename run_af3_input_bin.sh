...
mkdir "$WDIR/output/af3binary_bis/json"
echo "Working directory: $WDIR"
#------------ af3 binary reprediction:-----------------
# input preparation:
# getting the ids from the accepted2 binders:
shopt -s nocasematch
dirs=$(printf '%s\n' ./output/af3ternary/accepted3/*/ | sed 's:/$::; s:.*/::')
paths=()
for dir in $dirs; do
...
python "$SDIR/scripts/AF3_input_module.py"  --structure "${paths[@]}"  --target_id "$target_id" --target_template "$target_template"  --outdir "$WDIR/output/af3binary_bis/json"
